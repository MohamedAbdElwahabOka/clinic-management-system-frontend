...
export function LedgerTable({ entries, onEdit, onDelete }: LedgerTableProps) {
  ...
  const getSortIndicator = (key: SortKey) => {
    if (sortKey === key) {
      return sortOrder === 'asc' ? <ArrowUpDown className="h-3 w-3 ml-1 rtl:mr-1 rtl:ml-0 inline-block transform rotate-0" /> : <ArrowUpDown className="h-3 w-3 ml-1 rtl:mr-1 rtl:ml-0 inline-block transform rotate-180" />;
    }
    return <ArrowUpDown className="h-3 w-3 ml-1 rtl:mr-1 rtl:ml-0 inline-block opacity-30 group-hover:opacity-100" />;
  };
  ...
  const handleDeleteConfirm = (entry: LedgerEntry) => {
    ...
    toast({
      ...
      description: translate('ledgerEntryDeletedDesc', "Entry \"{{description}}\" has been removed.", { description: entry.description }),
    });
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{translate('ledgerEntriesDesc', "List of all recorded income and expense transactions.")}</CardDescription>
      </CardHeader>
      <CardContent>
        {sortedEntries.length > 0 ? (
              ...
              <TableBody>
                {sortedEntries.map((entry) => (
                  <TableRow key={entry.id}>
                    <TableCell>{format(parseISO(entry.date), "MMM d, yyyy", { locale: locale === 'ar' ? arSA : undefined })}</TableCell>
                    <TableCell className="font-medium max-w-xs truncate" title={entry.description}>{entry.description}</TableCell>
                    <TableCell>{entry.categoryName}</TableCell>
                    <TableCell>
                      <Badge variant={entry.type === 'income' ? 'default' : 'destructive'} className={entry.type === 'income' ? 'bg-green-600 hover:bg-green-700' : ''}>
                        {translate(entry.type, entry.type.charAt(0).toUpperCase() + entry.type.slice(1))}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right rtl:text-left flex">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" title={`${translate('delete', "Delete")} ${entry.description}`}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>{translate('confirmDeleteLedgerEntryTitle', "Are you sure?")}</AlertDialogTitle>
                            <AlertDialogDescription>
                              {translate('confirmDeleteLedgerEntryDesc', "This action cannot be undone. This will permanently delete the entry: \"{{description}}\".", {description: entry.description})}
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>{translate('cancel', "Cancel")}</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDeleteConfirm(entry)} className="bg-destructive hover:bg-destructive/90">{translate('delete', "Delete")}</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </TableCell>
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-lg font-medium text-muted-foreground">{translate('noLedgerEntries', "No ledger entries found.")}</p>
            <p className="text-sm text-muted-foreground">{translate('noLedgerEntriesDesc', "Use the form to add new income or expense transactions.")}</p>
          </div>
        )}
      </CardContent>
    </Card>
