  ...
  return (
    <div className="w-full max-w-sm">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button variant="link" asChild className="p-0 h-auto text-sm">
              <Link href="#">{translate('authForgotPasswordLink', 'Forgot Your Password?')}</Link>
            </Button>
          </div>
          <Button type="submit" className="w-full h-12 text-base" disabled={isLoading}>
            {isLoading ? translate('loggingIn') : <> <LogIn className="mr-2 h-5 w-5" /> {translate('loginButton')} </>}
          </Button>
        </form>
      </Form>
      ...
      <div className="flex items-center my-6">
        ...
        <span className="mx-4 text-xs text-muted-foreground uppercase">{translate('authOr', 'OR')}</span>
        <div className="flex-grow border-t border-muted"></div>
      </div>

      <div className="grid grid-cols-4 gap-2">
        <Button variant="outline" className="w-full h-12 aspect-square p-0" title={translate('authLoginWithApple', 'Log in with Apple')} disabled>
          <Apple className="h-5 w-5" /> <span className="sr-only">{translate('authApple', 'Apple')}</span>
        </Button>
        <Button variant="outline" className="w-full h-12 aspect-square p-0" title={translate('authLoginWithX', 'Log in with X')} disabled>
          <XIcon className="h-5 w-5" /> <span className="sr-only">{translate('authX', 'X')}</span>
        </Button>
        <Button variant="outline" className="w-full h-12 aspect-square p-0" title={translate('authLoginWithFacebook', 'Log in with Facebook')} disabled>
