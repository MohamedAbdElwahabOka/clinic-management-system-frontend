    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-20 transition-colors" dir="rtl">
        ...
        <div className="space-y-3">
          {filteredAppointments.length === 0 ? (
            ...
            filteredAppointments.map(appt => (
              <div 
                key={appt.id} 
                className="bg-white dark:bg-gray-900 rounded-xl p-4 border dark:border-gray-800 hover:shadow-md transition-shadow"
                onClick={() => setSelectedAppt(appt)}
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg">{appt.patientName}</h3>
                    {appt.phone && (
      ...
      {selectedAppt && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setSelectedAppt(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b dark:border-gray-800">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{selectedAppt.patientName}</h2>
                  {selectedAppt.phone && (
                    <a href={`tel:${selectedAppt.phone}`} className="text-blue-600 dark:text-blue-400 text-sm mt-1 flex items-center gap-1">
