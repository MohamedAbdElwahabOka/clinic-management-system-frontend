...
const isTestResultNormal = (test: { result: string; range?: string }): boolean => {
  ...
  return (
    <>
      ...
      {/* Table */}
      <div className="rounded-md border shadow-sm bg-card p-4 m-0 overflow-x-auto">
  ...
  >
    <div
      className={`bg-white dark:bg-gray-900 rounded-lg shadow-xl overflow-hidden flex flex-col 
        ${maximized ? "w-[100vw] h-[100vh]" : "w-full max-w-6xl h-[90vh] max-h-[90vh]"} transition-all duration-300`}
      onClick={(e) => e.stopPropagation()}
    >
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-3 sm:px-6 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-800 text-white gap-4">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          ...
          <Button size="icon" variant="ghost" className="text-white hover:bg-blue-700 h-8 w-8 sm:h-10 sm:w-10" onClick={handlePrint} aria-label="طباعة">
            <Printer size={18} />
          </Button>
          <Button size="icon" variant="ghost" className="text-white hover:bg-blue-700 h-8 w-8 sm:h-10 sm:w-10" onClick={handleExportPDF} aria-label="PDF">
            <Download size={18} />
          </Button>
          <Button size="icon" variant="ghost" className="text-white hover:bg-blue-700 h-8 w-8 sm:h-10 sm:w-10" onClick={() => setOpen(false)} aria-label="إغلاق">
            <X size={18} />
          </Button>
        </div>
      </div>
      ...
      <div
        ref={contentRef}
        dir="rtl"
        className="flex-1 overflow-auto p-4 sm:p-6 bg-gray-50 text-gray-900"
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          {/* المحتوى الرئيسي */}
          <div className="lg:col-span-9 space-y-5 order-2 lg:order-none">
            {/* علامات التبويب */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="flex border-b border-gray-200 overflow-x-auto scrollbar-hide">
                <button 
                  className={`px-5 py-3 font-medium whitespace-nowrap flex-shrink-0 ${
