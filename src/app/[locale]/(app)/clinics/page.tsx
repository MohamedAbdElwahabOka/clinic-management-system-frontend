...
export default function ClinicOperationsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 w-full sm:px-4 md:px-0" dir={isRTL ? "rtl" : "ltr"}>
      ...
      {/* 1. Header & Navigation */}
      <div className="mb-8">
        <Button variant="ghost" className="mb-4 text-gray-500 hover:text-gray-900 px-0 gap-2">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold text-gray-900">{t.manage}: {getLocalizedText(clinicData.name)}</h1>
              <Badge variant="secondary" className="text-base font-normal px-3 py-1 bg-white border-gray-200">
      ...
      <Tabs defaultValue="inventory" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-white border border-gray-200 p-1 rounded-xl h-auto w-full md:w-auto flex overflow-x-auto">
          <TabsTrigger value="inventory" className="flex-1 md:flex-none px-6 py-3 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 rounded-lg gap-2">
            <Package size={18} /> {t.tabs.inventory}
          </TabsTrigger>
          <TabsTrigger value="payroll" className="flex-1 md:flex-none px-6 py-3 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-700 rounded-lg gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="bg-white border-blue-100 shadow-sm">
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="text-sm text-gray-500 mb-1">{t.inv.totalItems}</p>
                  <h3 className="text-2xl font-bold text-blue-700">{inventory.length}</h3>
                </div>
                <div className="p-3 bg-blue-50 rounded-full text-blue-600"><Package /></div>
              </CardContent>
            </Card>
            <Card className="bg-white border-amber-100 shadow-sm">
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="text-sm text-gray-500 mb-1">{t.inv.lowStock}</p>
                  <h3 className="text-2xl font-bold text-amber-600">{inventory.filter(i => i.status !== 'good').length}</h3>
                </div>
                <div className="p-3 bg-amber-50 rounded-full text-amber-600"><AlertTriangle /></div>
              </CardContent>
            </Card>
            <Card className="bg-white border-red-100 shadow-sm">
              <CardContent className="p-4 flex justify-between items-center">
                <div>
                  <p className="text-sm text-gray-500 mb-1">{t.inv.wasted}</p>
                  <h3 className="text-2xl font-bold text-red-600">
                    {inventory.reduce((acc, curr) => acc + curr.wastedCount, 0)}
                  </h3>
                </div>
                <div className="p-3 bg-red-50 rounded-full text-red-600"><Trash2 /></div>
              </CardContent>
            </Card>
          </div>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {suggestedItems.map((item: any, idx: number) => (
                    <button key={idx} className="flex items-center gap-1 bg-white hover:bg-blue-100 border border-blue-200 text-blue-700 px-3 py-1.5 rounded-full text-sm transition-colors shadow-sm">
                      <Plus size={14} /> {getLocalizedText(item)}
                    </button>
                  ))}
                </div>
              </div>
              <Button className="shrink-0 gap-2 bg-blue-600 hover:bg-blue-700 text-white">
                ...
                <tbody className="divide-y divide-gray-100">
                  {inventory.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${item.status === 'critical' ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
                            <Package size={20} />
                          </div>
                          <div>
                            <div className="font-bold text-gray-900">{getLocalizedText(item.itemName)}</div>
                            <div className="text-xs text-gray-500">{getLocalizedText(item.category)}</div>
                          </div>
                        </div>
                      </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-gradient-to-br from-purple-50 to-white border-purple-100 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-purple-900 uppercase flex items-center gap-2">
              ...
              <CardContent>
                ...
                <p className="text-xs text-purple-500 mt-1">Estimated monthly cost</p>
              </CardContent>
            </Card>
            
            <Card className="bg-gradient-to-br from-amber-50 to-white border-amber-100 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-amber-900 uppercase flex items-center gap-2">
              ...
              <CardContent>
                ...
                <p className="text-xs text-amber-500 mt-1">Actions required immediately</p>
              </CardContent>
            </Card>
          </div>
                    ...
                    <tr key={staff.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-gray-900">{getLocalizedText(staff.name)}</div>
