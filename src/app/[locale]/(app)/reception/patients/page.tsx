  ...
  <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
    <div className="flex justify-between items-start mb-3">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold ${patient.gender === 'male' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
          {patient.name.charAt(0)}
        </div>
        <div>
          <h3 className="font-bold text-gray-900">{patient.name}</h3>
          <p className="text-xs text-gray-500">{patient.gender === 'male' ? 'ذكر' : 'أنثى'} • {patient.age} سنة</p>
        </div>
      </div>
    ...
    <div className="space-y-2 text-sm text-gray-600 mb-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={patient.insurance !== "خاص (نقدي)" ? "text-blue-600 font-medium" : ""}>{patient.insurance}</span>
      </div>
      <div className="flex items-center gap-2">
    ...
    <div className="flex gap-2 pt-3 border-t border-gray-50">
      <button className="flex-1 bg-blue-50 text-blue-700 py-2 rounded-lg text-sm font-bold hover:bg-blue-100 transition-colors">حجز موعد</button>
      <button className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-lg text-sm font-medium hover:bg-gray-50">الملف</button>
    </div>
  </div>
);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
                <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
                    <h3 className="text-lg font-bold">تسجيل مريض جديد</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600">✕</button>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">اسم المريض رباعي</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="tel" className="w-full border rounded-lg px-3 py-2 outline-none"
                                value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">السن</label>
                            <input type="number" className="w-full border rounded-lg px-3 py-2 outline-none"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">جهة التأمين</label>
                            <select className="w-full border rounded-lg px-3 py-2 outline-none bg-white"
                ...
                <div className="p-4 border-t bg-gray-50 flex justify-end gap-3">
                    <button onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-lg text-sm font-medium">إلغاء</button>
    ...
    <div dir="rtl" className="min-h-screen bg-gray-50/50 p-4 md:p-8 font-sans text-gray-900">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">إدارة ملفات المرضى والبحث السريع</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative flex-1 sm:w-80">
            <input 
              ...
              className="w-full pl-4 pr-10 py-2.5 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all"
            />
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-200 transition-all active:scale-95"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
            <p className="text-xs text-gray-500">مرضى اليوم</p>
            <p className="text-xl font-bold text-green-600">12</p>
        </div>
        <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
            <p className="text-xs text-gray-500">شركات التأمين</p>
        ...
        <table className="w-full text-right">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">السن / النوع</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">التأمين</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">آخر زيارة</th>
              <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">الإجراءات</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filteredPatients.map((patient) => (
              <tr key={patient.id} className="hover:bg-blue-50/50 group transition-colors">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${patient.gender === 'male' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
                        {patient.name.charAt(0)}
                    </div>
                    <div>
                      <p className="font-bold text-gray-900 text-sm">{patient.name}</p>
                      <p className="text-xs text-gray-500">ملف #{patient.id}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 text-sm font-mono text-gray-600" dir="ltr">{patient.phone}</td>
                <td className="px-6 py-4 text-sm text-gray-600">
                    {patient.age} سنة <span className="text-gray-300 mx-1">|</span> {patient.gender === 'male' ? 'ذكر' : 'أنثى'}
                </td>
                <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                        patient.insurance === "خاص (نقدي)" 
                ...
                <td className="px-6 py-4">
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-500" title="تعديل"><Edit className="w-4 h-4" /></button>
                        <button className="p-1.5 hover:bg-red-50 rounded-lg text-red-500" title="حذف"><Trash2 className="w-4 h-4" /></button>
                    </div>
                </td>
              </tr>
            ))}
